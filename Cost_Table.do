...
gen loss_adr_exypf=market_adr_usd*.545/1000
gen loss_exypf=market_usd*.545/1000
gen loss_ypf=market_ypf_usd*.9522/1000

gen loss_adr=loss_adr_exypf+loss_ypf
gen loss=loss_exypf+loss_ypf
keep loss*

foreach x in loss_adr_exypf loss_exypf loss_ypf loss_adr loss {
	gen `x'60=`x'*.6
	rename `x' `x'100
}
gen temp="temp"
reshape long loss_adr_exypf loss_exypf loss_ypf loss_adr loss, i(temp) j(num)
drop temp
